...
import com.aastudio.sarollahi.api.PERSON_MOVIE_ADS_PLACEMENT_ID
...
class PersonMoviesFragment : Fragment() {
    ...
    private fun setUpRecyclerView(adapter: MoviesAdapter) {
        ...
        myMoPubAdapter.loadAds(PERSON_MOVIE_ADS_PLACEMENT_ID)
    }

    companion object {
        private const val MOVIE_LIST = "movieList"

        fun newInstance(movieList: ArrayList<Movie>?): PersonMoviesFragment {
            val fragment = PersonMoviesFragment()
            val args = Bundle()
            args.putParcelableArrayList(MOVIE_LIST, movieList)
