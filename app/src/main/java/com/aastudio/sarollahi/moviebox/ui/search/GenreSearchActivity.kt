...
class GenreSearchActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (genreId != null && genreId != -1) {
            if (isMovie) {
                viewModel.findMoviesByGenre(page, sort, genreId!!)
                setUpRecyclerView(moviesAdapter)
            } else {
                viewModel.findTVShowsByGenre(page, sort, genreId!!)
        ...
        binding.searchContent.searchContainer.addOnScrollListener(object :
                ...
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    ...
                    if (firstVisibleItem + visibleItemCount >= totalItemCount / 2) {
                        ...
                        genreId?.let { id ->
                            if (isMovie) {
                                viewModel.findMoviesByGenre(page, sort, id)
                            } else {
                                viewModel.findTVShowsByGenre(page, sort, id)
                            }
                        }
                    }
                }
